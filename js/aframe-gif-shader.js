...
/***/ function(module, exports, __webpack_require__) {
	...
	AFRAME.registerShader('gif', {
	  ...
	  __validateSrc: function __validateSrc(src, cb) {
        log('__validateSrc', src);
	    /* check if src is a url */
	    var url = parseUrl(src);
        log('__validateSrc parsed url:', url);
	    if (url) {
	      this.__getImageSrc(url, cb);
	      return;
	    }

        /* Fallback: If parseUrl failed but it looks like a path (has / or . and doesn't start with #), try it as a URL */
        if (src && (src.indexOf('/') !== -1 || src.indexOf('.') !== -1) && src.indexOf('#') !== 0) {
            log('__validateSrc: parseUrl failed, but looks like a path. Trying directly.');
            this.__getImageSrc(src, cb);
            return;
        }
	  ...
	  __ready: function __ready(_ref) {
	    ...
	    this.el.emit('gif-loaded', { src: src }, true);
