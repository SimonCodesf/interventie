...
async function initializeARMode() {
    ...
    try {
        ...
        console.log('📋 Posters with AR markers:', postersWithMarkers.length);
        
        if (postersWithMarkers.length === 0) {
            console.warn('⚠️ No posters with AR markers found. AR features disabled, but UI active.');
            // Initialize static camera feed anyway so user sees camera
        ...
        for (const poster of postersWithMarkers) {
            ...
            try {
                ...
                if (testFetch.ok) {
                    ...
                    console.log(`✅ Valid .mind file: ${poster.title} → ${mindPath}`);
                } else {
                    console.warn(`⚠️ Missing .mind file for ${poster.title}: ${mindPath}`);
                }
            } catch (e) {
                console.warn(`⚠️ Error checking .mind file for ${poster.title}:`, e);
            }
        }
        
        if (validPosters.length === 0) {
            console.error('❌ No valid .mind files found!');
        ...
        console.log('🎯 AR Posters ready:', validPosters.map(p => p.title));
        ...
        for (const poster of validPosters) {
            try {
                ...
                if (response.ok) {
                    ...
                    console.log(`✅ Preloaded: ${poster.title} (${(arrayBuffer.byteLength / 1024).toFixed(1)} KB)`);
                }
            } catch (e) {
                console.warn(`⚠️ Failed to preload ${poster.title}:`, e);
            }
        }
        console.log(`📦 Preloaded ${preloadedMindData.size}/${validPosters.length} .mind files`);
        
        // Create blob URLs for faster loading during scanning
        createMindBlobUrls();
        console.log(`🔗 Created ${mindBlobUrls.size} blob URLs for instant loading`);
        
        // Pre-build all scene HTML for instant switching
        prebuildAllSceneHTML();
        
        // === MULTI-TARGET BATCH SYSTEM ===
        // Create batches of posters for parallel checking (2-3 per batch)
        const BATCH_SIZE = 2; // Number of .mind files per batch (safe limit for WebGL)
        window.posterBatches = [];
        for (let i = 0; i < validPosters.length; i += BATCH_SIZE) {
            const batch = validPosters.slice(i, i + BATCH_SIZE);
            window.posterBatches.push(batch);
        }
        console.log(`📦 Created ${window.posterBatches.length} batches of ${BATCH_SIZE} posters each`);
        
        // Track which batch is currently active
        ...
        console.log('🎯 Starting with poster:', validPosters[0].title);
        
        // STEP 1: Create static camera feed overlay (hides AR flicker)
        await initializeStaticCameraFeed();
        
        // STEP 2: Initialize AR scene (hidden behind static feed)
        initializeARScene();
        
        // Show SCAN button (scanner starts when user clicks)
        if (validPosters.length > 1) {
            console.log('📋 Multiple posters - showing SCAN button');
            setTimeout(() => {
                showScanButton();
            }, 2000);
        } else {
            console.log('📌 Only 1 poster, no scan needed');
        }
...
function initializeARScene() {
    ...
    scene.addEventListener('arReady', () => {
        ...
        // NOTE: fixLayerAspectRatios() removed - a-image handles aspect ratio automatically!
    ...
    if (poster.layers) {
        for (let i = 1; i <= 8; i++) {
            ...
            if (layerData && layerData.filename) {
                ...
                // Use a-plane with fixed dimensions - NO async aspect ratio fixing
                // Keep it simple: 1.4 height, proportional width
                layersHTML += `
                    <a-plane 
                        id="ar-layer-${i}"
                        src="${imagePath}" 
...
function showDetectedPosterDetails(poster) {
    ...
    if (!detailsContainer || !poster) {
        console.error('❌ Missing container or poster data');
        return;
    }
